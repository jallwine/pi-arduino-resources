...
void loop() {
    ...
    // Read whether the button is pushed or not (LOW or HIGH)
    buttonState = digitalRead(buttonpin);
    
    if(buttonState == HIGH) {
        noTone(buzzerPin);
    } else {
        float t = (float)flexposition/1023;
        tone(buzzerPin, t*low_frequency+(1-t)*high_frequency);
    }
